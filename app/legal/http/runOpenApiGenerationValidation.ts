...
async function main(): Promise<void> {
  ...
  const healthRoute = createHealthHttpRoute(healthController);
  ...
  const ragRoute = createRagHttpRoute(ragController);

  const generator = new OpenApiGenerator();
  const document = generator.generate([healthRoute, ragRoute]);

  assertTruthy(document.openapi, "openapi version missing");
  assertTruthy(document.info, "info missing");
  assertTruthy(document.info.title, "info title missing");
  assertTruthy(document.info.version, "info version missing");

  const healthPathItem = document.paths["/health"];
  assertTruthy(healthPathItem, "/health path missing");
  assertTruthy(healthPathItem.get, "GET operation missing for /health");
  assertTruthy(
    healthPathItem.get?.operationId,
    "operationId missing for GET /health",
  );
  assertTruthy(
    healthPathItem.get?.responses,
    "responses missing for GET /health",
  );

  const ragPathItem = document.paths["/rag/answer"];
  assertTruthy(ragPathItem, "/rag/answer path missing");
  assertTruthy(ragPathItem.post, "POST operation missing for /rag/answer");
  assertTruthy(
    ragPathItem.post?.operationId,
    "operationId missing for POST /rag/answer",
  );
  assertTruthy(
    ragPathItem.post?.responses,
    "responses missing for POST /rag/answer",
  );

  console.log("OpenAPI generation validation succeeded.");
}
