...
async function main(): Promise<void> {
  ...
  const route = createRagHttpRoute(ragController);

  assertEqual(route.method, "POST", "method mismatch");
  assertEqual(route.path, "/rag/answer", "path mismatch");
  ...
  const body = response.body as { answer: string; citations: unknown[] };
  if (typeof body.answer !== "string" || body.answer.length === 0) {
    throw new Error("response body missing non-empty answer field");
  }
  if (!Array.isArray(body.citations)) {
    throw new Error("response body missing citations array");
  }
  assertEqual(body.citations.length, 1, "citation count mismatch");

  console.log("RAG HTTP route validation succeeded.");
}
