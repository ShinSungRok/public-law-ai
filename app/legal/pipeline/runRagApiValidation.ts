...
async function main(): Promise<void> {
  ...
  const request: RagAnswerRequestDto = { query: SAMPLE_QUERY };
  const response = await ragController.answer(request);

  console.log(`Request: ${JSON.stringify(request)}`);
  console.log(`Answer: ${response.answer}`);
  console.log(`Citation count: ${response.citations.length}`);
  for (const citation of response.citations) {
    console.log(`Citation: ${citation.displayText} (${citation.sourceUrl})`);
  }
}
